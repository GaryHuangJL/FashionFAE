...
@registry.register_loss("contrastivefsis_loss")
class ContrastivefsisLoss(nn.Module):
    ...
    def forward(self, sample_list: Dict[str, Tensor], model_output: Dict[str, Tensor]):
        embedding_1 = model_output["scores"]  # image embedding
        embedding_2 = model_output["targets"]
        label = sample_list['fsislabel']
        ...
        label_all_gpus = gather_tensor_along_batch_with_backward(label)
        label_all_gpus = label_all_gpus.unsqueeze(0)
        embedding_1_all_gpus = gather_tensor_along_batch_with_backward(embedding_1)
        embedding_2_all_gpus = gather_tensor_along_batch_with_backward(embedding_2)
        labels_allgpu = torch.zeros(label_all_gpus.size(1), label_all_gpus.size(1), device=embedding_1.device)
        for i in range(label_all_gpus.size(1)):
            labels_allgpu[i, :] = label_all_gpus[:, i] - label_all_gpus

        logits_1 = (
            torch.matmul(embedding_1, embedding_2_all_gpus.transpose(0, 1))
            / self.temperature
        )
        logits_1 = (logits_1 + 1) / 2
        labels_all = labels_allgpu[per_gpu_batch_size * get_rank():per_gpu_batch_size * (get_rank()+1),:]
        labels_all[labels_all != 0] = 1
        labels_all1 = 1 - labels_all
        labels_zeros = torch.zeros(per_gpu_batch_size, per_gpu_batch_size, device=embedding_1.device)
        labels_match = torch.eye(per_gpu_batch_size, device=embedding_1.device)
        for i in range(3):
            if i < get_rank():
                labels_match = torch.cat((labels_zeros, labels_match), dim=1)
            else:
                labels_match = torch.cat((labels_match, labels_zeros), dim=1)

        labels_matchothers = labels_all1 - labels_match
        loss = logits_1 * labels_all + (1 - logits_1) * labels_match + (1 - logits_1) * labels_matchothers * 0.3
        loss = loss.mean()
