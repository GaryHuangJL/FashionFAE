...
@registry.register_metric("r@k_rank")
class Recall_average(BaseMetric):
    def __init__(self, name="recall_average"):
        super().__init__(name)
    ...
    def _get_rk(
        ...
        # acclerate sort with topk
        max_sim, indices = torch.topk(
            q_embeddings @ g_embeddings.t(), k=len(g_ids), dim=1, largest=True, sorted=True
        )  # q * k
        pred_labels = g_ids[indices]  # q * k
        matches = pred_labels.eq(q_ids.view(-1, 1))  # q * k

        row_indices = torch.nonzero((matches + 0) == 1, as_tuple=False)
        rows, cols = row_indices[:, 0], row_indices[:, 1]
        # 使用 unique 函数获取每一行的唯一行索引
        ...
        for row in unique_rows:
            ...
            row_average = torch.mean(row_cols.float()).item()
            row_averages.append(row_average)

        return sum(row_averages)/len(row_averages)+1
    ...
    def calculate(
        ...
        **kwargs,
        ...
        values = torch.zeros(1, device=image_embeddings.device)
        values[0] = self._get_rk(
...
class RecallAtK_kaleido(BaseMetric):
    ...
    def calculate(
        ...
        **kwargs,
        ...
        image_ids = torch.cat(all_gather_diff_size(image_ids), dim=0)
        text_ids = torch.cat(all_gather_diff_size(text_ids), dim=0)
        image_subcat_ids = torch.cat(all_gather_diff_size(image_subcat_ids), dim=0)
        text_subcat_ids = torch.cat(all_gather_diff_size(text_subcat_ids), dim=0)
